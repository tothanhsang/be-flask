...
# user_two_face = face_recognition.load_image_file('')
...
#     known_face_names.append(re.sub('[0-9'.'', filename[:-4]))
#     known_face_encodings.append(face_recognition.faces_encodings(face)[0])

# # Load picture 
# face_picture = face_recognition.load_image_file('')
...
#     best_match_index = np.argmin(face_distances)
#     if matches[best_match_index]:
#         name = known__face_names[best_match_index]
...
#             matches = face_recognition.compare_faces(known_face_encodings, face_encoding)
#             name = "Unknown"
#             face_dsitances = face_recognition.face_distance(known_face_encodings, face_encoding)
#             best_match_index = np.argmin(face_dsitances)
#             if matches[best_match_index]:
#                 name = known__face_names[best_match_index]
#                 json_to_export['name'] = name
#                 json_to_export['hour'] = f'{time.localtime().tm_hour}:{time.localtime().tm_min}'
#                 json_to_export['date'] = f'{time.localtime().tm_year}-{time.locatime().tm_mon}-{time.localtime().tm_day()'
#                 json_to_export['picture_array'] = frame.tolist()

#                 r = request.post(url='http://127.0.0.1:500/receive_data', json=json_to_export)
#                 print('Status: ', r.status_code)

#         face_names.append(name)
#         process_this_frame = not process_this_frame 
#         for (top, right, bottom, left), name in zip(face_locations, face_names):
#             cv2.reactangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)
#             font = cv2.FONT_HERSHEY_DUPLEX
#             cv2.putText(frame, name, (left + 6, bottom -6), font, 1.0, (255, 255, 255), 1)
#         cv2.imshow('Video',)
# video_capture.release()
